...
public class ElexisJdbcUserDetailsManager extends JdbcUserDetailsManager implements UserInfoRepository {
	...
	public ElexisJdbcUserDetailsManager() {
		...
		setUsersByUsernameQuery(
				"select id, concat(hashed_password,'|', salt), totp, is_active, is_administrator, allow_external from user_ where deleted = '0' and id=?");
		...
		return getJdbcTemplate().query(getUsersByUsernameQuery(), new String[] { username },
				new RowMapper<UserDetails>() {
					...
					public UserDetails mapRow(ResultSet rs, int rowNum) throws SQLException {
						...
						String totpSecret = rs.getString(3);
						boolean enabled = rs.getBoolean(4);
						boolean isAdmin = rs.getBoolean(5);
						boolean allowExternal = rs.getBoolean(6);
						List<GrantedAuthority> authorities = (isAdmin)
								? AuthorityUtils.createAuthorityList("ROLE_ADMIN", "ROLE_USER")
								: AuthorityUtils.createAuthorityList("ROLE_USER");
						return new OtpUser(username, password, allowExternal, true, true, enabled, authorities,
								totpSecret);
					}
	...
	/**
	 * we have to override this, as we already determine the ROLE_USER and
	 * ROLE_ADMIN while loading the user list.
	 */
	@Override
